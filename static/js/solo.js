...
minussButton.addEventListener("click", function () {
  ...
  if (currentValue > mins) {
    ...
    // Asegurar que Armed no sea mayor que Arms
    maxd = armsField.value - 1;
    if (parseInt(armedField.value) > armsField.value - 1) {
      armedField.value = armsField.value - 1;
    }
  }
...
plussButton.addEventListener("click", function () {
  ...
  if (currentValue < maxs) {
    ...
    // Asegurar que Armed pueda alcanzar a arms
    maxd = currentValue;
  }
